...
interface MissingRoleTranslationsModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  missingRoleIds: number[];
}

export const MissingRoleTranslationsModal: React.FC<MissingRoleTranslationsModalProps> = ({
  isOpen,
  onOpenChange,
  missingRoleIds,
}) => {
  ...
  const [processedRoles, setProcessedRoles] = useState<Set<number>>(new Set());

  const currentRoleId = missingRoleIds[currentIndex];
  const totalRoles = missingRoleIds.length;
  const progress = Math.round(((currentIndex + 1) / totalRoles) * 100);
  ...
  // Queries
  const { data: currentRole, isLoading: isRoleLoading } = useQuery({
    queryKey: ['role', currentRoleId],
    queryFn: async () => {
      const roles = await roleService.getRoles({ pageSize: 1000 });
      return roles.items.find(r => r.id === currentRoleId);
    },
    enabled: !!currentRoleId && isOpen,
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['roles-missing-translations'] });
      
      setProcessedRoles(prev => new Set([...prev, currentRoleId]));
      toast.success(
        isRTL 
  ...
  useEffect(() => {
    if (currentRole) {
      form.reset({
        ...
        localizations: currentRole.localizations?.map(loc => ({
          id: loc.id,
      ...
      replaceLocalizations(currentRole.localizations?.map(loc => ({
        id: loc.id,
        languageId: loc.languageId,
        name: loc.name,
        description: loc.description,
  ...
  }, [currentRole, form, replaceLocalizations]);

  // Navigation functions
  const goToNext = () => {
    if (currentIndex < totalRoles - 1) {
  ...
  const onSubmit = async (data: UpdateRoleDto) => {
    ...
    updateMutation.mutate({ id: currentRoleId, data }, {
      onSuccess: () => {
        if (currentIndex < totalRoles - 1) {
          goToNext();
        } else {
        ...
        <DialogHeader className="space-y-3 p-4 sm:p-6 pb-3 sm:pb-4 border-b flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:justify-between">
            <DialogTitle className="text-lg sm:text-xl flex items-center gap-2 sm:gap-3 order-1 sm:order-1">
              ...
              <span className="text-base sm:text-xl font-semibold leading-tight">
                {isRTL ? 'إضافة ترجمات الأدوار المفقودة' : 'Add Missing Role Translations'}
              </span>
            </DialogTitle>
            <Button
              variant="ghost"
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2 text-sm">
              <span className="text-muted-foreground text-xs sm:text-sm">
                {isRTL 
                  ? `الدور ${currentIndex + 1} من ${totalRoles}`
                  : `Role ${currentIndex + 1} of ${totalRoles}`
                }
              </span>
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-start sm:items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-xs sm:text-sm truncate">{currentRole.name}</h3>
                      <p className="text-xs text-muted-foreground line-clamp-1 sm:line-clamp-2 mt-0.5">
                        {currentRole.description || (isRTL ? 'لا يوجد وصف' : 'No description')}
                      </p>
                    </div>
                    <Badge variant="secondary" className="text-xs flex-shrink-0">
                      <span className="hidden xs:inline">
                        {currentRole.localizations?.length || 0} {isRTL ? 'ترجمة' : 'translations'}
                      </span>
                      <span className="xs:hidden">
                        {currentRole.localizations?.length || 0}
                      </span>
                    </Badge>
                  </div>
                </CardContent>
          ...
          <DialogDescription className="flex items-start gap-2 text-xs sm:text-sm">
            ...
            <span className="leading-relaxed">
              {isRTL 
                ? 'أضف ترجمات الأدوار للغات مختلفة لتحسين تجربة المستخدم متعدد اللغات'
                : 'Add role translations in different languages to improve multilingual user experience'
              }
            </span>
          </DialogDescription>
        </DialogHeader>
                    ...
                    <Card className="border-dashed border-2 border-muted-foreground/20">
                      <CardContent className="py-8 sm:py-12 text-center px-4 sm:px-6">
                        ...
                        <p className="text-muted-foreground text-xs sm:text-sm mb-4 leading-relaxed">
                          {isRTL 
                            ? 'أضف ترجمات لهذا الدور لتحسين تجربة المستخدم'
                            : 'Add translations for this role to improve user experience'
                          }
                        </p>
                        <Button
                          type="button"
                      ...
                      <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-6 pb-3 sm:pb-6">
                        <div className="grid grid-cols-1 gap-3 sm:gap-4">
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs sm:text-sm">{isRTL ? 'اسم الدور' : 'Role Name'}</FormLabel>
                                <FormControl>
                                  <Input
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs sm:text-sm">{isRTL ? 'وصف الدور' : 'Role Description'}</FormLabel>
                                <FormControl>
                                  <Textarea
                                    className="min-h-[60px] sm:min-h-[72px] text-xs sm:text-sm resize-none"
                                    placeholder={isRTL ? 'وصف الدور بهذه اللغة' : 'Role description in this language'}
                                    rows={2}
                                    {...field}
                                  />
                                </FormControl>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-3">
            ...
            <div className="flex items-center gap-2 order-2 sm:order-1">
              ...
              <Button
                ...
                size="sm"
                onClick={goToNext}
                disabled={currentIndex >= totalRoles - 1}
                className="h-8 sm:h-9 px-2 sm:px-3 text-xs sm:text-sm flex-1 sm:flex-initial"
              >
...
export default MissingRoleTranslationsModal;
