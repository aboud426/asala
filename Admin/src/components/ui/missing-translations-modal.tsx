...
export const MissingTranslationsModal: React.FC<MissingTranslationsModalProps> = ({
  isOpen,
  onOpenChange,
  missingCategoryIds,
}) => {
  ...
  const [processedCategories, setProcessedCategories] = useState<Set<number>>(new Set());

  const currentCategoryId = missingCategoryIds[currentIndex];
  const totalCategories = missingCategoryIds.length;
  const progress = Math.round(((currentIndex + 1) / totalCategories) * 100);
  ...
  const { data: currentCategory, isLoading: isCategoryLoading } = useQuery({
    ...
    queryFn: async () => {
      const categories = await categoryService.getCategories({ pageSize: 1000 });
      return categories.items.find(c => c.id === currentCategoryId);
    },
    enabled: !!currentCategoryId && isOpen,
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['categories-missing-translations'] });
      
      setProcessedCategories(prev => new Set([...prev, currentCategoryId]));
      toast.success(
        isRTL 
  ...
  useEffect(() => {
    if (currentCategory) {
      form.reset({
        ...
        localizations: currentCategory.localizations?.map(loc => ({
          id: loc.id,
      ...
      replaceLocalizations(currentCategory.localizations?.map(loc => ({
        id: loc.id,
        nameLocalized: loc.nameLocalized,
        descriptionLocalized: loc.descriptionLocalized,
        languageId: loc.languageId,
  ...
  }, [currentCategory, form, replaceLocalizations]);
  ...
  const handleClose = () => {
    setCurrentIndex(0);
    setProcessedCategories(new Set());
    onOpenChange(false);
    form.reset();
  };
  ...
  const onSubmit = async (data: UpdateCategoryDto) => {
    ...
    updateMutation.mutate({ id: currentCategoryId, data }, {
      onSuccess: () => {
        if (currentIndex < totalCategories - 1) {
          goToNext();
        } else {
        ...
        <DialogHeader className="space-y-3 p-4 sm:p-6 pb-3 sm:pb-4 border-b flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:justify-between">
            <DialogTitle className="text-lg sm:text-xl flex items-center gap-2 sm:gap-3 order-1 sm:order-1">
              ...
              <span className="text-base sm:text-xl font-semibold leading-tight">
                {isRTL ? 'إضافة الترجمات المفقودة' : 'Add Missing Translations'}
              </span>
            </DialogTitle>
            <Button
              variant="ghost"
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2 text-sm">
              <span className="text-muted-foreground text-xs sm:text-sm">
                {isRTL 
                  ? `الفئة ${currentIndex + 1} من ${totalCategories}`
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-start sm:items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-xs sm:text-sm truncate">{currentCategory.name}</h3>
                    ...
                    <Badge variant="secondary" className="text-xs flex-shrink-0">
                      <span className="hidden xs:inline">
                        {currentCategory.localizations?.length || 0} {isRTL ? 'ترجمة' : 'translations'}
                      </span>
                      <span className="xs:hidden">
                        {currentCategory.localizations?.length || 0}
                      </span>
                    </Badge>
                  </div>
                </CardContent>
          ...
          <DialogDescription className="flex items-start gap-2 text-xs sm:text-sm">
            ...
            <span className="leading-relaxed">
              {isRTL 
                ? 'أضف ترجمات للغات مختلفة لتحسين تجربة المستخدم متعدد اللغات'
                : 'Add translations in different languages to improve multilingual user experience'
              }
            </span>
          </DialogDescription>
        </DialogHeader>
                    ...
                    <Card className="border-dashed border-2 border-muted-foreground/20">
                      <CardContent className="py-8 sm:py-12 text-center px-4 sm:px-6">
                        ...
                        <p className="text-muted-foreground text-xs sm:text-sm mb-4 leading-relaxed">
                          {isRTL 
                            ? 'أضف ترجمات لهذه الفئة لتحسين تجربة المستخدم'
                            : 'Add translations for this category to improve user experience'
                          }
                        </p>
                        <Button
                          type="button"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-3">
            ...
            <div className="flex items-center gap-2 order-1 sm:order-2">
              ...
              <Button
                ...
                disabled={updateMutation.isPending || isCategoryLoading}
              >
