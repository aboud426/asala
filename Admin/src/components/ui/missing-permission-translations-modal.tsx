...
interface MissingPermissionTranslationsModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  missingPermissionIds: number[];
}

export const MissingPermissionTranslationsModal: React.FC<MissingPermissionTranslationsModalProps> = ({
  isOpen,
  onOpenChange,
  ...
  const [processedPermissions, setProcessedPermissions] = useState<Set<number>>(new Set());

  const currentPermissionId = missingPermissionIds[currentIndex];
  const totalPermissions = missingPermissionIds.length;
  const progress = Math.round(((currentIndex + 1) / totalPermissions) * 100);
  ...
  // Queries
  const { data: currentPermission, isLoading: isPermissionLoading } = useQuery({
    queryKey: ['permission', currentPermissionId],
    queryFn: async () => {
      const permissions = await permissionService.getPermissions({ pageSize: 1000 });
      return permissions.items.find(p => p.id === currentPermissionId);
    },
    enabled: !!currentPermissionId && isOpen,
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['permissions-missing-translations'] });
      
      setProcessedPermissions(prev => new Set([...prev, currentPermissionId]));
      toast.success(
        isRTL 
  ...
  useEffect(() => {
    if (currentPermission) {
      form.reset({
        ...
        localizations: currentPermission.localizations?.map(loc => ({
          id: loc.id,
      ...
      replaceLocalizations(currentPermission.localizations?.map(loc => ({
        id: loc.id,
        languageId: loc.languageId,
        name: loc.name,
        description: loc.description,
  ...
  }, [currentPermission, form, replaceLocalizations]);

  // Navigation functions
  const goToNext = () => {
    if (currentIndex < totalPermissions - 1) {
  ...
  const handleClose = () => {
    setCurrentIndex(0);
    setProcessedPermissions(new Set());
    onOpenChange(false);
    form.reset();
  };

  // Form submission
  const onSubmit = async (data: UpdatePermissionDto) => {
    if (!currentPermissionId) return;

    updateMutation.mutate({ id: currentPermissionId, data }, {
      onSuccess: () => {
        if (currentIndex < totalPermissions - 1) {
          goToNext();
        } else {
        ...
        <DialogHeader className="space-y-3 p-4 sm:p-6 pb-3 sm:pb-4 border-b flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:justify-between">
            <DialogTitle className="text-lg sm:text-xl flex items-center gap-2 sm:gap-3 order-1 sm:order-1">
              ...
              <span className="text-base sm:text-xl font-semibold leading-tight">
                {isRTL ? 'إضافة ترجمات الصلاحيات المفقودة' : 'Add Missing Permission Translations'}
              </span>
            </DialogTitle>
            <Button
              variant="ghost"
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2 text-sm">
              <span className="text-muted-foreground text-xs sm:text-sm">
                {isRTL 
                  ? `الصلاحية ${currentIndex + 1} من ${totalPermissions}`
                  : `Permission ${currentIndex + 1} of ${totalPermissions}`
                }
              </span>
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-start sm:items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-xs sm:text-sm truncate">{currentPermission.name}</h3>
                      <p className="text-xs text-muted-foreground line-clamp-1 sm:line-clamp-2 mt-0.5">
                        {currentPermission.description || (isRTL ? 'لا يوجد وصف' : 'No description')}
                      </p>
                    </div>
                    <Badge variant="secondary" className="text-xs flex-shrink-0">
                      <span className="hidden xs:inline">
                        {currentPermission.localizations?.length || 0} {isRTL ? 'ترجمة' : 'translations'}
                      </span>
                      <span className="xs:hidden">
                        {currentPermission.localizations?.length || 0}
                      </span>
                    </Badge>
                  </div>
                </CardContent>
          ...
          <DialogDescription className="flex items-start gap-2 text-xs sm:text-sm">
            ...
            <span className="leading-relaxed">
              {isRTL 
                ? 'أضف ترجمات الصلاحيات للغات مختلفة لتحسين تجربة المستخدم متعدد اللغات'
                : 'Add permission translations in different languages to improve multilingual user experience'
              }
            </span>
          </DialogDescription>
        </DialogHeader>
                    ...
                    <Card className="border-dashed border-2 border-muted-foreground/20">
                      <CardContent className="py-8 sm:py-12 text-center px-4 sm:px-6">
                        ...
                        <p className="text-muted-foreground text-xs sm:text-sm mb-4 leading-relaxed">
                          {isRTL 
                            ? 'أضف ترجمات لهذه الصلاحية لتحسين تجربة المستخدم'
                            : 'Add translations for this permission to improve user experience'
                          }
                        </p>
                        <Button
                          type="button"
                      ...
                      <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-6 pb-3 sm:pb-6">
                        <div className="grid grid-cols-1 gap-3 sm:gap-4">
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs sm:text-sm">{isRTL ? 'اسم الصلاحية' : 'Permission Name'}</FormLabel>
                                <FormControl>
                                  <Input
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs sm:text-sm">{isRTL ? 'وصف الصلاحية' : 'Permission Description'}</FormLabel>
                                <FormControl>
                                  <Textarea
                                    className="min-h-[60px] sm:min-h-[72px] text-xs sm:text-sm resize-none"
                                    placeholder={isRTL ? 'وصف الصلاحية بهذه اللغة' : 'Permission description in this language'}
                                    rows={2}
                                    {...field}
                                  />
                                </FormControl>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-3">
            ...
            <div className="flex items-center gap-2 order-2 sm:order-1">
              ...
              <Button
                ...
                size="sm"
                onClick={goToNext}
                disabled={currentIndex >= totalPermissions - 1}
                className="h-8 sm:h-9 px-2 sm:px-3 text-xs sm:text-sm flex-1 sm:flex-initial"
              >
            ...
            <div className="flex items-center gap-2 order-1 sm:order-2">
              ...
              <Button
                ...
                disabled={updateMutation.isPending || isPermissionLoading}
              >
...
export default MissingPermissionTranslationsModal;
