...
interface MissingPostTypeTranslationsModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  missingPostTypeIds: number[];
}

export const MissingPostTypeTranslationsModal: React.FC<MissingPostTypeTranslationsModalProps> = ({
  isOpen,
  onOpenChange,
  missingPostTypeIds,
}) => {
  ...
  const [processedPostTypes, setProcessedPostTypes] = useState<Set<number>>(new Set());

  const currentPostTypeId = missingPostTypeIds[currentIndex];
  const totalPostTypes = missingPostTypeIds.length;
  const progress = Math.round(((currentIndex + 1) / totalPostTypes) * 100);

  // Form setup
  const form = useForm<UpdatePostTypeDto>({
    defaultValues: {
      name: '',
      description: '',
      isActive: true,
  ...
  // Queries
  const { data: currentPostType, isLoading: isPostTypeLoading } = useQuery({
    queryKey: ['postType', currentPostTypeId],
    queryFn: async () => {
      const postTypes = await postTypeService.getPostTypes({ pageSize: 1000 });
      return postTypes.items.find(pt => pt.id === currentPostTypeId);
    },
    enabled: !!currentPostTypeId && isOpen,
  });
  ...
  // Mutation for updating post type
  const updateMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['postTypes-missing-translations'] });
      
      setProcessedPostTypes(prev => new Set([...prev, currentPostTypeId]));
      toast.success(
        isRTL 
  ...
  useEffect(() => {
    if (currentPostType) {
      form.reset({
        ...
        isActive: currentPostType.isActive,
        localizations: currentPostType.localizations?.map(loc => ({
          id: loc.id,
          name: loc.name,
      ...
      replaceLocalizations(currentPostType.localizations?.map(loc => ({
        id: loc.id,
        name: loc.name,
        description: loc.description,
        languageId: loc.languageId,
  ...
  }, [currentPostType, form, replaceLocalizations]);

  // Navigation functions
  const goToNext = () => {
    if (currentIndex < totalPostTypes - 1) {
  ...
  const handleClose = () => {
    setCurrentIndex(0);
    setProcessedPostTypes(new Set());
    onOpenChange(false);
    form.reset();
  };

  // Form submission
  const onSubmit = async (data: UpdatePostTypeDto) => {
    if (!currentPostTypeId) return;

    updateMutation.mutate({ id: currentPostTypeId, data }, {
      onSuccess: () => {
        if (currentIndex < totalPostTypes - 1) {
          goToNext();
        } else {
          // All post types processed
          toast.success(
            isRTL 
        ...
        <DialogHeader className="space-y-3 p-4 sm:p-6 pb-3 sm:pb-4 border-b flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:justify-between">
            <DialogTitle className="text-lg sm:text-xl flex items-center gap-2 sm:gap-3 order-1 sm:order-1">
              ...
              <span className="text-base sm:text-xl font-semibold leading-tight">
                {isRTL ? 'إضافة ترجمات أنواع المنشورات المفقودة' : 'Add Missing Post Type Translations'}
              </span>
            </DialogTitle>
            <Button
              variant="ghost"
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2 text-sm">
              <span className="text-muted-foreground text-xs sm:text-sm">
                {isRTL 
                  ? `نوع المنشور ${currentIndex + 1} من ${totalPostTypes}`
                  : `Post Type ${currentIndex + 1} of ${totalPostTypes}`
                }
              </span>
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-start sm:items-center gap-3">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gradient-primary flex items-center justify-center flex-shrink-0">
                      <Tag className="h-3 w-3 sm:h-4 sm:w-4 text-primary-foreground" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-xs sm:text-sm truncate">{currentPostType.name}</h3>
                      <p className="text-xs text-muted-foreground line-clamp-1 sm:line-clamp-2 mt-0.5">
                        {currentPostType.description || (isRTL ? 'لا يوجد وصف' : 'No description')}
                      </p>
                    </div>
                    <Badge variant="secondary" className="text-xs flex-shrink-0">
                      <span className="hidden xs:inline">
                        {currentPostType.localizations?.length || 0} {isRTL ? 'ترجمة' : 'translations'}
                      </span>
                      <span className="xs:hidden">
                        {currentPostType.localizations?.length || 0}
                      </span>
                    </Badge>
                  </div>
                </CardContent>
          ...
          <DialogDescription className="flex items-start gap-2 text-xs sm:text-sm">
            ...
            <span className="leading-relaxed">
              {isRTL 
                ? 'أضف ترجمات أنواع المنشورات للغات مختلفة لتحسين تجربة المستخدم متعدد اللغات'
                : 'Add post type translations in different languages to improve multilingual user experience'
              }
            </span>
          </DialogDescription>
        </DialogHeader>
                    ...
                    <Card className="border-dashed border-2 border-muted-foreground/20">
                      <CardContent className="py-8 sm:py-12 text-center px-4 sm:px-6">
                        ...
                        <p className="text-muted-foreground text-xs sm:text-sm mb-4 leading-relaxed">
                          {isRTL 
                            ? 'أضف ترجمات لنوع المنشور هذا لتحسين تجربة المستخدم'
                            : 'Add translations for this post type to improve user experience'
                          }
                        </p>
                        <Button
                          type="button"
                              ...
                              <FormItem className="lg:col-span-1">
                                ...
                                <FormControl>
                                  <Input
                                    ...
                                    placeholder={isRTL ? 'اسم نوع المنشور بهذه اللغة' : 'Post type name in this language'}
                                    {...field}
                                  />
                                </FormControl>
                                <FormMessage className="text-xs" />
                            ...
                            <FormItem>
                              ...
                              <FormControl>
                                <Textarea
                                  ...
                                  placeholder={isRTL ? 'وصف نوع المنشور بهذه اللغة' : 'Post type description in this language'}
                                  rows={2}
                                  {...field}
                                />
                              </FormControl>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-3">
            ...
            <div className="flex items-center gap-2 order-2 sm:order-1">
              ...
              <Button
                ...
                size="sm"
                onClick={goToNext}
                disabled={currentIndex >= totalPostTypes - 1}
                className="h-8 sm:h-9 px-2 sm:px-3 text-xs sm:text-sm flex-1 sm:flex-initial"
              >
            ...
            <div className="flex items-center gap-2 order-1 sm:order-2">
              ...
              <Button
                ...
                disabled={updateMutation.isPending || isPostTypeLoading}
              >
...
export default MissingPostTypeTranslationsModal;
