...
interface MissingCurrencyTranslationsModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  missingCurrencyIds: number[];
}

export const MissingCurrencyTranslationsModal: React.FC<MissingCurrencyTranslationsModalProps> = ({
  isOpen,
  onOpenChange,
  missingCurrencyIds,
}) => {
  ...
  const [processedCurrencies, setProcessedCurrencies] = useState<Set<number>>(new Set());

  const currentCurrencyId = missingCurrencyIds[currentIndex];
  const totalCurrencies = missingCurrencyIds.length;
  const progress = Math.round(((currentIndex + 1) / totalCurrencies) * 100);

  // Form setup
  const form = useForm<UpdateCurrencyDto>({
    defaultValues: {
      name: '',
      code: '',
      symbol: '',
  ...
  // Queries
  const { data: currentCurrency, isLoading: isCurrencyLoading } = useQuery({
    queryKey: ['currency', currentCurrencyId],
    queryFn: async () => {
      const currencies = await currencyService.getCurrencies({ pageSize: 1000 });
      return currencies.items.find(c => c.id === currentCurrencyId);
    },
    enabled: !!currentCurrencyId && isOpen,
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setProcessedCurrencies(prev => new Set([...prev, currentCurrencyId]));
      toast.success(
        isRTL 
  ...
  useEffect(() => {
    if (currentCurrency) {
      form.reset({
        ...
        localizations: currentCurrency.localizations?.map(loc => ({
          id: loc.id,
      ...
      replaceLocalizations(currentCurrency.localizations?.map(loc => ({
        id: loc.id,
        languageId: loc.languageId,
        name: loc.name,
        code: loc.code,
  ...
  }, [currentCurrency, form, replaceLocalizations]);

  // Navigation functions
  const goToNext = () => {
    if (currentIndex < totalCurrencies - 1) {
  ...
  const handleClose = () => {
    setCurrentIndex(0);
    setProcessedCurrencies(new Set());
    onOpenChange(false);
    form.reset();
  };

  // Form submission
  const onSubmit = async (data: UpdateCurrencyDto) => {
    if (!currentCurrencyId) return;

    updateMutation.mutate({ id: currentCurrencyId, data }, {
      onSuccess: () => {
        if (currentIndex < totalCurrencies - 1) {
          goToNext();
        } else {
        ...
        <DialogHeader className="space-y-3 p-4 sm:p-6 pb-3 sm:pb-4 border-b flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:justify-between">
            <DialogTitle className="text-lg sm:text-xl flex items-center gap-2 sm:gap-3 order-1 sm:order-1">
              ...
              <span className="text-base sm:text-xl font-semibold leading-tight">
                {isRTL ? 'إضافة ترجمات العملات المفقودة' : 'Add Missing Currency Translations'}
              </span>
            </DialogTitle>
            <Button
              variant="ghost"
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2 text-sm">
              <span className="text-muted-foreground text-xs sm:text-sm">
                {isRTL 
                  ? `العملة ${currentIndex + 1} من ${totalCurrencies}`
                  : `Currency ${currentIndex + 1} of ${totalCurrencies}`
                }
              </span>
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-start sm:items-center gap-3">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gradient-primary flex items-center justify-center flex-shrink-0">
                      <Coins className="h-3 w-3 sm:h-4 sm:w-4 text-primary-foreground" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-xs sm:text-sm truncate">{currentCurrency.name}</h3>
                    ...
                    <Badge variant="secondary" className="text-xs flex-shrink-0">
                      <span className="hidden xs:inline">
                        {currentCurrency.localizations?.length || 0} {isRTL ? 'ترجمة' : 'translations'}
                      </span>
                      <span className="xs:hidden">
                        {currentCurrency.localizations?.length || 0}
                      </span>
                    </Badge>
                  </div>
                </CardContent>
          ...
          <DialogDescription className="flex items-start gap-2 text-xs sm:text-sm">
            ...
            <span className="leading-relaxed">
              {isRTL 
                ? 'أضف ترجمات العملات للغات مختلفة لتحسين تجربة المستخدم متعدد اللغات'
                : 'Add currency translations in different languages to improve multilingual user experience'
              }
            </span>
          </DialogDescription>
        </DialogHeader>
                    ...
                    <Card className="border-dashed border-2 border-muted-foreground/20">
                      <CardContent className="py-8 sm:py-12 text-center px-4 sm:px-6">
                        ...
                        <p className="text-muted-foreground text-xs sm:text-sm mb-4 leading-relaxed">
                          {isRTL 
                            ? 'أضف ترجمات لهذه العملة لتحسين تجربة المستخدم'
                            : 'Add translations for this currency to improve user experience'
                          }
                        </p>
                        <Button
                          type="button"
                      ...
                      <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-6 pb-3 sm:pb-6">
                        <div className="grid grid-cols-1 gap-3 sm:gap-4">
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs sm:text-sm">{isRTL ? 'اسم العملة' : 'Currency Name'}</FormLabel>
                                <FormControl>
                                  <Input
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs sm:text-sm">{isRTL ? 'رمز العملة' : 'Currency Code'}</FormLabel>
                                <FormControl>
                                  <Input
                                    className="h-8 sm:h-9 text-xs sm:text-sm"
                                    placeholder={isRTL ? 'رمز العملة بهذه اللغة' : 'Currency code in this language'}
                                    {...field}
                                  />
                                </FormControl>
                                <FormMessage className="text-xs" />
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs sm:text-sm">{isRTL ? 'رمز العملة' : 'Currency Symbol'}</FormLabel>
                                <FormControl>
                                  <Input
                                    className="h-8 sm:h-9 text-xs sm:text-sm"
                                    placeholder={isRTL ? 'رمز العملة بهذه اللغة' : 'Currency symbol in this language'}
                                    {...field}
                                  />
                                </FormControl>
                                <FormMessage className="text-xs" />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-3">
            ...
            <div className="flex items-center gap-2 order-2 sm:order-1">
              ...
              <Button
                ...
                size="sm"
                onClick={goToNext}
                disabled={currentIndex >= totalCurrencies - 1}
                className="h-8 sm:h-9 px-2 sm:px-3 text-xs sm:text-sm flex-1 sm:flex-initial"
              >
            ...
            <div className="flex items-center gap-2 order-1 sm:order-2">
              ...
              <Button
                ...
                disabled={updateMutation.isPending || isCurrencyLoading}
              >
...
export default MissingCurrencyTranslationsModal;
