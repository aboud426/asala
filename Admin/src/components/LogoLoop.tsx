...
const useRTLDetection = () => {
  ...
  useEffect(() => {
    const detectRTL = () => {
      ...
      if (documentDir) {
        return documentDir.toLowerCase() === 'rtl';
      }
...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      if (seqWidth > 0) {
        ...
        // In RTL mode, we need to flip the transform direction
        const translateX = isRTL ? offsetRef.current : -offsetRef.current;
        track.style.transform = `translate3d(${translateX}px, 0, 0)`;
      }
  ...
  }, [targetVelocity, seqWidth, isHovered, pauseOnHover, isRTL, trackRef]);
};
...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    const targetVelocity = useMemo(() => {
      ...
      // In RTL mode, flip the direction logic to maintain expected behavior
      if (isRTL) {
    ...
    useAnimationLoop(trackRef, targetVelocity, seqWidth, isHovered, pauseOnHover, isRTL);
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        ...
        if (isNodeItem) {
          ...
          const itemAriaLabel = nodeItem.ariaLabel ?? nodeItem.title;

          const inner = nodeItem.href ? (
            <a
              className={cx(
          ...
          const itemAriaLabel = imageItem.alt ?? imageItem.title;

          const inner = imageItem.href ? (
            <a
              className={cx(
                'inline-flex items-center no-underline rounded',
                'transition-opacity duration-200 ease-linear',
        ...
        {fadeOut && (
          <>
            <div
              aria-hidden
              className={cx(
                'pointer-events-none absolute inset-y-0 z-[1]',
